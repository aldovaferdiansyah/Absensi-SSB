...
class QrScannerController extends Controller
{
    ...
    // Code logika untuk melakukan absensi siswa oleh pelatih menggunakan tombol
    ...
    private function recordAttendanceManual($user, $role)
    {
        try {
            $today = Carbon::today();

            $todaySchedule = Schedule::whereDate('date', $today)->first();

            if (!$todaySchedule) {
                return response()->json(['error' => 'Jadwal tidak ditemukan untuk hari ini.'], 404);
            }

            $type = $todaySchedule->title;
